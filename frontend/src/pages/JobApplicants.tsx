...
const JobApplicants: React.FC = () => {
    ...
    useEffect(() => {
        const fetchApplicants = async (): Promise<void> => {
            try {
                ...
                const res = await axios.get<Applicant[]>(`/api/applications/${jobId}`, config);
                setApplicants(res.data);
            } catch (err) {
                const axiosError = err as AxiosError<ErrorResponse>;
                setError(axiosError.response?.data?.message || 'Error fetching applicants');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleViewResume = (resumeUrl: string): void => {
        if (!resumeUrl) {
